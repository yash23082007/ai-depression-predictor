...
const getRecommendations = (formData, isHighRisk) => {
    ...
    if (formData.sleep_duration === 'Less than 5 Hours' || formData.sleep_duration === '5-6 Hours') {
        tips.push({
            icon: <FaBed className="text-stone-500 mt-1 flex-shrink-0 text-xl" />,
    ...
    if (formData.dietary_habits === 'Unhealthy' || formData.dietary_habits === 'Moderate') {
        tips.push({
            icon: <FaAppleAlt className="text-sage-600 mt-1 flex-shrink-0 text-xl" />,
    ...
    if (tips.length === 0) {
        tips.push({
            icon: <FaCheckCircle className="text-sage-600 mt-1 flex-shrink-0 text-xl" />,
...
const ResultDashboard = () => {
    ...
    const chartData = {
        ...
        datasets: [{
            ...
            backgroundColor: [
                isHighRisk ? '#ef4444' : '#1c1917', // red-500 or stone-900
                '#e7e5e4', // stone-200
            ],
            borderWidth: 0,
            circumference: 180,
    ...
    return (
        <div ref={containerRef} className="max-w-5xl mx-auto flex flex-col items-center py-16 px-4 min-h-[90vh]">
            <h2 className="text-4xl md:text-5xl font-serif font-medium text-stone-900 mb-12 tracking-tight text-center">
                ...
                <div ref={scoreCardRef} className="md:col-span-2 premium-card p-10 flex flex-col items-center justify-center relative bg-white">
                    <div className="w-full max-w-[260px] h-36 mb-6 relative">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-end pb-3">
                            <span className={`text-5xl font-serif font-medium tracking-tight ${isHighRisk ? 'text-red-500' : 'text-stone-900'}`}>
                                {risk_score}%
                            </span>
                        </div>
                    </div>
                    
                    <p className="text-stone-400 mt-2 text-[10px] font-bold uppercase tracking-widest">Strain Indicator</p>

                    <div className={`mt-8 px-6 py-2.5 rounded-xl font-semibold text-sm flex items-center gap-3 border shadow-crisp ${isHighRisk ? 'bg-red-50 text-red-600 border-red-200' : 'bg-stone-100 text-stone-700 border-stone-200'}`}>
                        {isHighRisk ? <FaExclamationTriangle className="text-red-500 text-base" /> : <FaCheckCircle className="text-stone-900 text-base" />}
                        {label === 'High' ? 'Needs Attention' : 'Looking Good'}
                    </div>

                    {formData.name && (
                        <p className="text-stone-400 text-[10px] font-bold mt-8 uppercase tracking-widest">
                            Prepared for <span className="text-stone-700">{formData.name}</span>
                        </p>
                    )}
                </div>

                {/* Advice Section */}
                <div className="md:col-span-3 premium-card p-8 sm:p-10 flex flex-col bg-stone-50/50">
                    <h3 className="text-2xl font-serif font-medium text-stone-900 mb-3 tracking-tight">Insights & Guidance</h3>
                    <p className="text-stone-500 mb-8 leading-relaxed font-medium text-sm">
                        Based on your daily patterns, you might be experiencing a <strong className={isHighRisk ? 'text-red-500' : 'text-stone-900 font-bold'}>{label.toLowerCase()}</strong> amount of emotional strain right now. Here are some gentle suggestions.
                    </p>
                    ...
                    <div ref={tipsRef} className="space-y-4 max-h-[380px] overflow-y-auto pr-3 custom-scrollbar">
                        {recommendations.map((rec, i) => (
                            <div key={i} className={`p-6 rounded-2xl border-l-[3px] border-t border-r border-b shadow-crisp flex gap-5 transition-transform hover:-translate-y-0.5 ${rec.border} ${rec.bg}`}>
                                ...
                                <div>
                                    <h4 className="font-semibold text-stone-900 text-base mb-1.5 tracking-wide">{rec.title}</h4>
            ...
            <div className="mt-12 flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
                <Link to="/assessment" className="w-full md:w-auto px-8 py-4 rounded-xl font-semibold text-stone-600 bg-stone-100 border border-stone-200 shadow-sm hover:bg-stone-200 hover:text-stone-900 transition-all flex items-center justify-center gap-3 transform active:scale-95">
                    <FaRedoAlt className="text-stone-400" /> Revisit Check-in
                </Link>
                <Link to="/" className="w-full md:w-auto px-8 py-4 bg-stone-900 text-stone-50 rounded-xl font-semibold hover:bg-stone-800 transition-all flex items-center justify-center gap-3 shadow-card transform hover:-translate-y-0.5 active:scale-95">
                    <FaHome className="mb-0.5 text-stone-400" /> Return to Home
                </Link>
            </div>
        </div>
    );
